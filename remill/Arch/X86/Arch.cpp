...
static bool IsConditionalBranch(const xed_decoded_inst_t *xedd) {
  ...
  LOG(INFO)                                                                      
      << "inst category is conditional branch : "<< xed_decoded_inst_get_category(xedd);  // changes done
}
...
static Instruction::Category CreateCategory(const xed_decoded_inst_t *xedd) {
  ...
  // denote that the interrupt should happen.
    ...
    LOG(INFO)                                                                      
      << "inst category directjumpfar return is :\n "<< xed_decoded_inst_get_category(xedd);  // changes done           
    return Instruction::kCategoryAsyncHyperCall;
...
static std::string InstructionFunctionName(const xed_decoded_inst_t *xedd) {
  ...
  LOG(INFO)                                                                   // changes added
      << " the inst form is :"<< xed_decoded_inst_get_iform_enum(xedd);
  ...
  LOG(INFO)                                                                       // changes added
      << " the inst form name  is :"<< xed_iform_enum_t2str(iform); ;
  ss << iform_name;
...
static bool DecodeXED(xed_decoded_inst_t *xedd,
  ...
  LOG(INFO)
      << " num of bytes are :"<< inst_bytes.size();
  ...
  LOG(INFO)
      << " decoded bytes are :"<< xed_decode(xedd, bytes, static_cast<uint32_t>(num_bytes));
...
static void DecodeMemory(Instruction &inst,
  ...
  auto iclass = xed_decoded_inst_get_iclass(xedd);
  LOG(INFO)                                                                   // changes added
      << " the inst class is : " <<  iclass;
  auto op_name = xed_operand_name(xedo);
  LOG(INFO)
      << " the operand name is  :"<< op_name;
  auto segment = xed_decoded_inst_get_seg_reg(xedd, mem_index); 
  LOG(INFO)                                                                           // changes added
      << " the segment is : " << xed_decoded_inst_get_seg_reg(xedd, mem_index);
...
static void DecodeRegister(Instruction &inst,
  ...
  LOG(INFO)
      << " register is :  "<< xed_decoded_inst_get_reg(xedd, op_name);    // changes added
...
static void DecodeFallThroughPC(Instruction &inst,
  ...
  LOG(INFO)
      << " the pc register is :" << pc_reg;                       // changes added
...
bool X86Arch::DecodeInstruction(
  ...
  LOG(INFO)
      << " architecture is : "<< inst.arch_name;
  inst.category = Instruction::kCategoryInvalid;
  LOG(INFO)
      << " category is : "<< inst.category;
...
bool X86Arch::LazyDecodeInstruction(
  ...
  if (DecodeInstruction(address, inst_bytes, inst, true)) {
    LOG(INFO)
        << " decoded inst is :" << DecodeInstruction(address, inst_bytes, inst, true);  // changes added
