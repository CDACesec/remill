...
LiftStatus InstructionLifter::LiftIntoBlock(
  ...
  auto arg_num = 2U;
  auto isel_func_number = isel_func_type->getNumParams();
  auto arch_inst_function = arch_inst.function;
...
bool TraceLifter::Lift(uint64_t addr_,
  ...
  while (!state.trace_work_list.empty()) {
    ...
    while (!state.inst_work_list.empty()) {
      ...
      auto lift_status = inst_lifter.LiftIntoBlock(state.inst, state.block, false);
